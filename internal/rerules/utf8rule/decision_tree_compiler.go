...
func (self *DecisionTreeCompiler) Feed(rule ggfnt.Utf8RewriteRule, index RuleIndex) error {
	if !self.compiling { panic(PreViolation) } // discretionary safety assertion
	...
	// for each rune set in the rule, for each range in the rune set, append it
	err := self.run.Scanner.Start(rule)
	if err != nil { return err }

	for self.run.Scanner.HasNext() {
		codePoint, utf8Set, err := self.run.Scanner.Next()
		if err != nil { return err }
		...
		for _, state := range self.run.FromStates {
			...
			// append rune or rune range
			if codePoint != RuneError { // code point case
				if utf8Set != Utf8SetNone { panic(BrokenCode) } // discretionary safety assertion
	
				runeRange := RuneRange{ First: codePoint, Last: codePoint }
				err := self.states[state].AppendRange(runeRange, matchRule, self.stateLinker)
				if err != nil { return err }
			} else { // utf8 set case
				if utf8Set == Utf8SetNone { panic(BrokenCode) } // discretionary safety assertion
	
				set := self.run.Font.Rewrites().GetUtf8Set(uint8(utf8Set))
				err := set.EachRange(func(firstRune, lastRune rune) error {
					runeRange := RuneRange{ First: firstRune, Last: lastRune }
